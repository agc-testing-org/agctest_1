class Account
    ...
    def mail_invite token
        ...
        on_team = get_seat invite[:user_id], invite.team_id # on team that sent invite / share
        if on_team
        ...
        begin
            ...
            notifications.each_with_index do |notification,i| 
                ...
                if notification.sprint_timeline.notification.name == "comment" #|| notification.sprint_timeline.notification.name == "vote"
                    ...
                    subject = "New Wired7 #{notification.sprint_timeline.notification.name.capitalize}"
                    html_body = "#{notification.user.first_name},<br><br>There's a new #{notification.sprint_timeline.notification.name} from #{profile} on the #{sprint_state} phase of the <i>#{project}</i> sprint <i>#{sprint}</i><br><br>\"#{notification.sprint_timeline.comment.text}\"<br><br>Use the following link to reply:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                    ...
                    subject = "Wired7 #{notification.sprint_timeline.state.name} transition"
                    html_body = "#{notification.user.first_name},<br><br>We've just started the #{notification.sprint_timeline.state.name} phase of the <i>#{project}</i> sprint <i>#{sprint}</i><br><br>#{notification.sprint_timeline.state.instruction}<br><br>Use the following link to join in:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                    ...
                    if notification.sprint_timeline.contributor.user.id == notification.user.id
                        ...
                        html_body = "#{notification.user.first_name},<br><br>Congratulations!  Your proposal for the #{notification.sprint_timeline.next_sprint_state.state.name} phase of the <i>#{project}</i> sprint <i>#{sprint}</i> has been selected for merge!  Use the following link to check out the other proposals:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                        ...
                        html_body = "#{notification.user.first_name},<br><br>A winning proposal has been selected for the #{notification.sprint_timeline.next_sprint_state.state.name} phase of the <i>#{project}</i> sprint <i>#{sprint}</i>.  Use the following link to check out all of the proposals:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                    ...
                    if notification.sprint_timeline.user_id == notification.user_id 
                        ...
                        subject = "Wired7 Idea Pitch for #{notification.sprint_timeline.job.title} at #{notification.sprint_timeline.job.team.company}"
                        html_body = "#{notification.user.first_name},<br><br>#{profile} has just proposed a new sprint idea for the #{notification.sprint_timeline.job.title} at #{notification.sprint_timeline.job.team.company} listing using <i>#{project}</i>:<br><br>#{sprint}<br><br>Use the following link to check it out:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                    ...
                    if notification.sprint_timeline.user_id == notification.user.id
                        ...
                        subject = "#{notification.sprint_timeline.job.team.name} at #{notification.sprint_timeline.job.team.company} is looking for a #{notification.sprint_timeline.job.title} on Wired7"
                        html_body = "#{notification.user.first_name},<br><br>#{notification.sprint_timeline.job.team.name} at #{notification.sprint_timeline.job.team.company} started a search for a #{notification.sprint_timeline.job.title}.  If you're interested, use the following link to propose and build an idea that earns the hiring manager's attention:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
                    ...
                    link = "#{ENV['INTEGRATIONS_HOST']}/develop/#{notification.sprint_timeline.project.id}-#{notification.sprint_timeline.project.org}-#{notification.sprint_timeline.project.name}/sprint/#{notification.sprint_timeline.sprint.id}-#{notification.sprint_timeline.sprint.title}/state/#{notification.sprint_timeline.sprint_state.id}-#{notification.sprint_timeline.state.name}"
                    subject = "Wired7 #{notification.sprint_timeline.state.name} deadline set"
                    html_body = "#{notification.user.first_name},<br><br>We've just set deadline for the #{notification.sprint_timeline.state.name} phase of the <i>#{project}</i> sprint <i>#{sprint}</i><br><br>At least three solutions have been proposed. Contribute in the next 48 hours to showcase your talent:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}"
