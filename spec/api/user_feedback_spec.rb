...
describe "/user-feedback" do
    ...
    shared_examples_for "user_feedback" do
        it "should return id" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["id"]).to eq(feedback_result["id"])
            end
        end
        it "should return sprint_id" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["sprint_id"]).to eq(feedback_result["sprint_id"])
            end
        end
        it "should return state" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["state_id"]).to eq(feedback_result["state_id"])
            end
        end
        it "should return project" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["project_id"]).to eq(feedback_result["project_id"])
            end
        end
        it "should return count" do
             expect(@feedback["meta"]["count"]).to eq @feedback_count
        end
    end
    ...
    shared_examples_for "user_feedback_comment" do
        ...
        it "should return comment" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["comment_id"]).to eq(feedback_result["comment_id"])
            end
        end
    end
    ...
    shared_examples_for "user_feedback_vote" do
        ...
        it "should return vote" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["vote_id"]).to eq(feedback_result["vote_id"])
            end
        end
    end
    ...
    shared_examples_for "user_feedback_contributor" do
        it "should return contributor" do
            @feedback_results.each_with_index do |feedback_result, i|
                expect(@feedback["data"][i]["contributor_id"]).to eq(feedback_result["contributor_id"])
            end
        end
    end
            ...
            context "filter by" do
                context "skillset_id" do
                    before(:each) do
                        ...
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT sprint_timelines.* FROM sprint_timelines INNER JOIN votes ON (sprint_timelines.vote_id = votes.id and sprint_timelines.vote_id IS NOT NULL) INNER JOIN sprint_states ON sprint_states.id = votes.sprint_state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) WHERE sprint_timelines.user_id = #{decrypt(@user_id).to_i} AND user_skillsets.skillset_id = #{@skillset_id} GROUP BY sprint_timelines.id ORDER BY sprint_timelines.created_at DESC"
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
                    end
                    it_behaves_like "ok"
                ...
                context "skillset_id&role_id" do
                    before(:each) do
                        ...
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT sprint_timelines.* FROM sprint_timelines INNER JOIN votes ON (sprint_timelines.vote_id = votes.id and sprint_timelines.vote_id IS NOT NULL) INNER JOIN sprint_states ON sprint_states.id = votes.sprint_state_id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE sprint_timelines.user_id = #{decrypt(@user_id).to_i} AND user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} GROUP BY sprint_timelines.id ORDER BY sprint_timelines.created_at DESC"
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
                    end
                    it_behaves_like "ok"
                ...
                context "paging" do
                    before(:each) do
                        @page = 2
                        get "/users/#{@user_id}/aggregate-votes", {:skillset_id => @skillset_id, :role_id => @role_id, :page => @page}.to_json
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT sprint_timelines.* FROM sprint_timelines INNER JOIN votes ON (sprint_timelines.vote_id = votes.id and sprint_timelines.vote_id IS NOT NULL) INNER JOIN sprint_states ON sprint_states.id = votes.sprint_state_id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE sprint_timelines.user_id = #{decrypt(@user_id).to_i} AND user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} GROUP BY sprint_timelines.id ORDER BY sprint_timelines.created_at DESC" 
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page} offset #{(@page - 1) * @per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
            ...
            context "filter by" do
                context "skillset_id" do
                    before(:each) do
                        ...
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT contributors.* FROM contributors INNER JOIN sprint_states ON contributors.sprint_state_id = sprint_states.id INNER JOIN sprints ON sprint_states.sprint_id = sprints.id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) WHERE user_skillsets.skillset_id = #{@skillset_id} AND contributors.user_id = #{decrypt(@user_id).to_i} GROUP BY contributors.id ORDER BY contributors.created_at DESC" 
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
                    end
                    it_behaves_like "ok"
                ...
                context "skillset_id&role_id" do
                    before(:each) do
                        ...
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT contributors.* FROM contributors INNER JOIN sprint_states ON contributors.sprint_state_id = sprint_states.id INNER JOIN sprints ON sprint_states.sprint_id = sprints.id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} AND contributors.user_id = #{decrypt(@user_id).to_i} GROUP BY contributors.id ORDER BY contributors.created_at DESC"
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
                    end
                    it_behaves_like "ok"
                ...
                context "paging" do
                    before(:each) do
                        @page = 2
                        get "/users/#{@user_id}/aggregate-contributors", {:skillset_id => @skillset_id, :role_id => @role_id, :page => @page}.to_json
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT contributors.* FROM contributors INNER JOIN sprint_states ON contributors.sprint_state_id = sprint_states.id INNER JOIN sprints ON sprint_states.sprint_id = sprints.id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} AND contributors.user_id = #{decrypt(@user_id).to_i} GROUP BY contributors.id ORDER BY contributors.created_at DESC" 
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page} offset #{(@page - 1) * @per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
        ...
        context "user_id" do
            ...
            context "filter by" do
                ...
                context "paging" do
                    before(:each) do
                        @page = 2
                        get "/users/#{@user_id}/aggregate-comments-received", {:skillset_id => @skillset_id, :role_id => @role_id, :page => @page}.to_json
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT sprint_timelines.* FROM sprint_timelines INNER JOIN contributors ON (sprint_timelines.contributor_id = contributors.id AND contributors.user_id != sprint_timelines.user_id) INNER JOIN comments ON (sprint_timelines.comment_id = comments.id and sprint_timelines.comment_id IS NOT NULL) INNER JOIN sprint_states ON sprint_states.id = comments.sprint_state_id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} AND contributors.user_id = #{decrypt(@user_id).to_i} GROUP BY sprint_timelines.id ORDER BY sprint_timelines.created_at DESC" 
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page} offset #{(@page - 1) * @per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
        ...
        context "user_id" do
            ...
            context "filter by" do
                ...
                context "paging" do
                    before(:each) do
                        @page = 2
                        get "/users/#{@user_id}/aggregate-votes-received", {:skillset_id => @skillset_id, :role_id => @role_id, :page => @page}.to_json
                        @feedback = JSON.parse(last_response.body)
                        base_query = "SELECT sprint_timelines.* FROM sprint_timelines INNER JOIN contributors ON (sprint_timelines.contributor_id = contributors.id AND contributors.user_id != sprint_timelines.user_id) INNER JOIN votes ON (sprint_timelines.vote_id = votes.id and sprint_timelines.vote_id IS NOT NULL) INNER JOIN sprint_states ON sprint_states.id = votes.sprint_state_id INNER JOIN role_states ON sprint_states.state_id = role_states.state_id LEFT JOIN sprint_skillsets ON sprint_skillsets.sprint_id = sprint_states.sprint_id LEFT JOIN user_skillsets ON (user_skillsets.skillset_id = sprint_skillsets.skillset_id AND user_skillsets.active = 1) LEFT JOIN user_roles ON (user_roles.role_id = role_states.role_id AND user_roles.active = 1) WHERE user_skillsets.skillset_id = #{@skillset_id} AND user_roles.role_id = #{@role_id} AND contributors.user_id = #{decrypt(@user_id).to_i} GROUP BY sprint_timelines.id ORDER BY sprint_timelines.created_at DESC"
                        @feedback_results = @mysql_client.query("#{base_query} limit #{@per_page} offset #{(@page - 1) * @per_page}")
                        @feedback_count = @mysql_client.query(base_query).count
