        ...
        begin
            ...
            notifications.each_with_index do |notification,i| 
                ...
                if notification.sprint_timeline.notification.name == "comment" #|| notification.sprint_timeline.notification.name == "vote"
                    ...
                    link = "#{ENV['INTEGRATIONS_HOST']}/develop/roadmap/job/#{notification.sprint_timeline.job.id}-#{notification.sprint_timeline.job.title}-at-#{notification.sprint_timeline.job.team.name}"
                    if notification.sprint_timeline.user_id == notification.user.id
                        #TODO send confirmation / email with information on next steps
                        return true
                    else
                        return mail notification.user.email, "#{notification.sprint_timeline.job.team.name} is looking for a #{notification.sprint_timeline.job.title} on Wired7", "#{notification.user.first_name},<br><br>A hiring manager at #{notification.sprint_timeline.job.team.name} started a search for a #{notification.sprint_timeline.job.title} on Wired7.  If you're interested, use the following link to propose and build an idea that earns the hiring manager's attention:<br><br><a href='#{link}'>#{link}</a><br><br><br>#{signature}", "#{notification.user.first_name},\n\nA hiring manager at #{notification.sprint_timeline.job.team.name} started a search for a #{notification.sprint_timeline.job.title} on Wired7.  If you're interested, use the following link to propose and build an idea that earns the hiring manager's attention:\n\n#{link}\n\n\n#{signature}"
                    end
                end
            end
        rescue => e
